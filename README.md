...
WorldSim is a headless C++20 world-simulation kernel designed for a persistent, planet-scale game world whose simulation fidelity changes with spatial LOD without changing the authoritative world model. The included vertical slice covers geography, a reduced coupled energy/moisture climate with land/ocean thermal inertia, closed planetary water exchange and snow-albedo feedback, a conservative atmosphere/ocean/terrestrial carbon cycle with logarithmic CO2 radiative forcing, basin hydrology with snow, shallow groundwater, delayed river routing and lake storage, living-soil detritus with persistent fast/slow soil carbon, explicit organic/mineral nitrogen cycling, finite-N plant limitation and a closed reduced atmosphere/ocean/land nitrogen loop, grass/shrub/tree plant functional types with local dispersal, competition and snow burial, fuel/moisture/wind/snow-driven wildfire with explicit fire-carbon transfers, carbon/nitrogen-accounted cohort fauna with trophic biomass bounds and local habitat-selected migration, and an independent magic domain. It also includes a stable C ABI and a Godot 4.7 GDExtension host project.
...
No changes to `Simulation`, `WorldState`, LOD code, snapshot framing, or engine adapters are required merely to introduce the domain. See `docs/ADDING_MODULES.md`. The authoritative reduced carbon-cycle contract is documented in `docs/CARBON_CYCLE.md`; terrestrial nitrogen limitation and accounting are documented in `docs/NITROGEN_CYCLE.md`.
