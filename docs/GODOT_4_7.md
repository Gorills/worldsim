...
equirectangular UV. Camera forward is converted from Godot scene axes using +X =
east and -Z = north; a second projected point 50 km ahead supplies the arrow
...
The scene keeps terrain/collision authoritative while projecting the living surface into terrain colors and visual-only near vegetation; distant spherical terrain and a sea-level ocean are presentation-only. See `docs/TERRAIN_SLICE.md` and `docs/LIVING_SURFACE.md` for the current boundaries.
