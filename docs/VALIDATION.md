...
- nitrogen-limited living soil plus planetary closure: carbon decomposition
  mineralizes tracked organic N, finite mineral N limits plant production,
  runoff/fire enter explicit current boundary fluxes, and atmosphere/ocean
  reservoirs return N through fixation/deposition without changing global N;
...
snow-cover suppression, deterministic natural ignition, elapsed-time scaling of the daily
burn cap, explicit fire-carbon transfer closure, PFT aggregate consistency,
coarse-to-fine spread through the active-cover resolver, extensive-ledger LOD
conservation and current snapshot round-trip.
...
bounded smoke and all 14 audit scenarios. Godot 4.7.2 rebuilt and its native
smoke plus Russian simulation-laboratory integration discovered all 62 fields,
including fire state. Exact scope and limitations are recorded in `FIRE.md`.
...
`docs/NITROGEN_CYCLE.md`. Fertility is a diagnostic of mineral-N density. The reduced global N store
models N2 donor, reactive deposition and ocean return boxes, but not resolved
atmospheric chemistry; phosphorus and persistent fauna N are still absent.
...
the grazing-timestep correction epoch 23, the magic forcing timebase
correction epoch 24, and the wildfire burn-cap timebase correction uses the
current epoch 25.
