...
Fluvial incision and water-independent hillslope transport are separate closures. The geology pass first builds a strictly downhill routing graph from the current surface, accumulates contributing land area and runoff in descending-elevation order, and uses that accumulated discharge rather than only local rainfall/runoff to drive stream-power-like incision. Fluvial incision is exactly zero when accumulated runoff is zero. Soil-mantled slopes can still move material by a reduced creep closure whose local transport rate grows with downhill slope and saturates with available mobile regolith depth. This follows the established slope-dependent hillslope-transport form and depth-limited soil availability used by landscape-evolution models, while deliberately avoiding a claim that the coarse one-hop WorldSim rate is a calibrated finite-volume diffusivity.
