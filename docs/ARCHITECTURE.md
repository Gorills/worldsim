...
`CubeSphereTopology::neighbors4()` remains a same-level topological query. Adaptive hierarchy-region operations use `WorldState::resolve_active_cover(region)`, which resolves an arbitrary hierarchy region to the active cell or active descendants that currently represent it and returns deterministic area-normalized weights summing to one. `WorldState::active_neighbors4(cell)` applies that region contract to the four same-level neighbor regions of an active source cell. These weights are for region aggregation and must not be reinterpreted as physical face geometry.

Spatial processes that cross an actual cell boundary use the separate `WorldState::active_face_neighbors4(cell)` contract. Each returned `ActiveFacePart` is an active coarse/fine leaf that physically touches the requested source face and carries the spherical shared-interface length in metres. `CubeSphereTopology::shared_boundary_length_m()` handles non-conforming and cube-face interfaces; `edge_length_m()` provides the direct single-cell edge metric. A refined neighboring hierarchy region therefore contributes only its boundary subfaces, not interior descendants.

The current world cover does not require a 2:1 balance constraint: both region and physical-face resolvers support an active coarse ancestor or multiple active descendants. This is a deliberate non-conforming AMR-style contract. p4est treats coarse/fine non-conforming faces and balancing as separate concerns, while deal.II likewise distinguishes whole faces from refined subfaces. The derived physical-face neighborhood is cached only for the current active cover and is invalidated whenever refinement or coarsening changes that cover.

References:
- p4est HOWTO and mesh/ghost practice: https://p4est.github.io/p4est-howto.pdf
- p4est balancing API: https://p4est.github.io/api/p4est-2.8.6/p4est__extended_8h.html
- deal.II `FEInterfaceValues` face/subface interface: https://dealii.org/9.5.0/doxygen/deal.II/classFEInterfaceValues.html
...
**Extensive** quantities scale with area/population and are conserved across LOD by split/sum. Examples: water volume, vegetation carbon, tracked nitrogen stocks, mana energy, ore stock, stored food, treasury money when spatially attributed.

**Intensive** quantities describe local state independent of represented area. They copy on refinement and use area-weighted aggregation on coarsening. Examples: temperature, humidity ratio, and the mineral-N-density-derived fertility diagnostic.
...
`ClimateStore` owns a separate uniform base-level transport graph with land and
ocean heat reservoirs, atmospheric water, a closed ocean-water reservoir and
global atmospheric/ocean carbon reservoirs. Carbon is kept on the same fixed
reference store rather than duplicated across adaptive leaves; ecology reports
daily extensive land-atmosphere fluxes and the store performs conservative
air-sea exchange before projecting CO2 diagnostics. It projects diagnostics to
active leaves while retaining its initial thermal geometry. Surface exchange area-integrates the current land mask only to split
precipitation between hydrology and ocean. Focus LOD therefore cannot create
heat, water or carbon stocks, and the climate graph never mistakes same-level
`neighbors4()` results for adaptive active neighbors. See `CLIMATE.md` and
`CARBON_CYCLE.md`.

Snow remains an extensive hydrology field. Climate area-integrates that stock
back to each fixed reference node, derives bounded fractional cover and blends
land albedo before radiation. The same projected cover gates reduced
vegetation exposure and fire danger. Focus LOD can split/sum snow storage but
cannot change reference-node cover or heat forcing; see `SNOW_ALBEDO.md`.
...
Terrestrial nitrogen remains ordinary extensive field state for litter,
fast/slow soil, mineral and per-PFT vegetation pools.
`ecology.vegetation_nitrogen_kg` and `ecology.soil_fertility` are derived
diagnostics. Planetary nitrogen v2 adds a non-spatial `NitrogenStore` owning
reduced atmospheric N2, reactive-atmosphere and ocean dissolved-N reservoirs.
Daily leaching/fire rate fields transfer into that store; fixation and
deposition return N to mineral fields. Cohort fauna additionally owns derived N
at a fixed reduced body C:N, so trophic growth/mortality transfers N through
the same persistent cohort biomass used by the carbon budget. Cumulative
leaching and fire-emission fields remain audit histories and are excluded from
the planetary stock to avoid double counting. Focus LOD therefore cannot create
or destroy global N. See `NITROGEN_CYCLE.md`.
...
- commands addressed to a hierarchy region resolve to every active cell currently representing it;
...
Snapshot v39 is the current compatibility epoch for authoritative world state. Version 2 may contain the old fixed-continent terrain, version 3 the pre-orogenic tectonic terrain, version 4 the pre-diversification plate layout, version 5 the unconstrained diversified layout, version 6 the minimum-separation static-geography model before persistent geology state, version 7 the first stateful-geology model before sediment mass used burial-dependent compaction, version 8 the compacted-sediment model before fluvial incision was separated from water-independent hillslope creep, version 9 the separated geomorphology model before regolith production became depth-dependent, version 10 the depth-dependent-regolith model before hillslope transport gained critical-slope acceleration, version 11 the critical-slope model before terrestrial drainage terminated at sea level and submerged sediment gained a dedicated marine-routing closure, version 12 the coast-to-basin geology model before living-soil ecology, version 13 the living-soil model before persistent grass/shrub/tree functional-type pools, and version 14 the Flora-v1 model before habitat-selected fauna redistribution. Version 15 predates volume-weighted crust restriction, oceanic buoyancy response, corrected seasons and bounded vegetation losses. Version 16 predates basin hydrology, its persistent routing graph and integrated cross-domain water fluxes. Version 17 predates the persistent climate heat/moisture reservoirs and closed planet-water exchange. Version 18 predates authoritative wildfire state, carbon-transfer ledgers and the fire-before-fauna ordering. Version 19 predates persistent fast/slow soil carbon and its explicit respiration flux/ledger. Version 20 predates reference-grid snow-cover aggregation, snow-dependent albedo and snow gating of vegetation/fire. Version 21 predates fauna carbon accounting, its respiration ledger and trophic biomass bounds. Version 22 predates elapsed-time scaling of the herbivore grazing ceiling. Version 23 predates elapsed-time scaling of the periodic magic forcing phase. Version 24 predates elapsed-time scaling of the daily wildfire burn ceiling. Version 25 predates elapsed-time scaling of wildfire active-fire persistence. Version 26 predates LOD-invariant field-impulse routing across all active cells representing a hierarchy region. Version 27 predates parent-consistent LOD exit hysteresis for legal coarse configurations. Version 28 predates resolution-aware, geometry-scaled lateral climate heat transport. Version 29 predates the authoritative atmosphere/ocean carbon reservoirs, explicit daily ecology carbon fluxes and CO2 radiative forcing. Version 30 predates explicit organic/mineral/vegetation nitrogen stocks, nitrogen-limited plant production and nitrogen boundary ledgers. Version 31 predates the global atmospheric/ocean nitrogen reservoirs, current boundary-rate fields and fixation/deposition return loop. Version 32 predates cohort-derived fauna nitrogen, food-N-limited animal growth and trophic N recycling. Version 33 predates area-integrated coarse coastal land fractions and therefore can resume with different terrestrial area semantics from identical bytes. Version 34 predates the fixed-support orographic elevation used by climate moisture forcing and ClimateStore v4. Version 35 predates fixed-support stochastic weather forcing, so an identical coarse snapshot would otherwise continue under a different random spatial support. Version 36 predates fixed-support natural-fire ignition, so coarse or adaptively refined snapshots could otherwise continue with a different ignition process from identical bytes. Version 37 predates flexed fixed-support coastal restriction, so identical coarse snapshots could otherwise continue with a different represented coastline. Version 38 predates physical-face spatial ecology, so identical snapshots could otherwise continue with different flora dispersal, wildfire spread and fauna migration semantics. Versions 2 through 38 are rejected because silently accepting them could mix incompatible authoritative world semantics. Long-term save compatibility should be implemented as explicit snapshot migrations. Do not silently deserialize old bytes into a changed model.
