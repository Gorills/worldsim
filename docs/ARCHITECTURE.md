...
Nitrogen cycle v1 keeps litter, fast/slow soil, mineral and per-PFT vegetation
nitrogen as ordinary extensive fields. `ecology.vegetation_nitrogen_kg` and
`ecology.soil_fertility` are derived diagnostics; the former is reconstructed
from PFT N and the latter from mineral-N density after soil/vegetation/LOD
updates. Cumulative leaching and fire-emission fields are boundary ledgers, not
active nutrient stocks. See `NITROGEN_CYCLE.md`.
