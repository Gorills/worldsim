...
ocean heat reservoirs, atmospheric water and a closed ocean-water reservoir.
It projects diagnostics to active leaves while retaining its initial thermal
geometry. Surface exchange area-integrates the current land mask only to split
precipitation between hydrology and ocean. Focus LOD therefore cannot create
heat or water stocks, and the climate graph never mistakes same-level
`neighbors4()` results for adaptive active neighbors. See `CLIMATE.md`.
